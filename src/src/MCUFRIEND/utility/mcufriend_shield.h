...
//modified for BikeHub version 1 by Balazs Sari
//LCD pins  |D7 |D6 |D5 |D4 |D3 |D2 |D1  |D0 | |RD |WR |RS |CS  |RST|
//STM32 pin |PA3|PA2|PA1|PA0|PB7|PB6|PA10|PA9| |PB1|PB0|PA7|PA15|PA8|
...
#define write_8(d) { \
    GPIOA->REGS(BSRR) = 0b0000011000001111 << 16; \
    GPIOB->REGS(BSRR) = 0b0000000011000000 << 16; \
    GPIOA->REGS(BSRR) = (((d) & 1<<0) << 9)  \
                      ...
                      | (((d) & 1<<6) >> 4)  \
                      | (((d) & 1<<7) >> 4); \
    GPIOB->REGS(BSRR) = (((d) & 1<<2) << 4)  \
                      | (((d) & 1<<3) << 4); \
}
//#define read_8()      (GPIOA->REGS(IDR) & 0xFF)
//                                         PA7 ..PA0
#define read_8()( ( ((GPIOA->REGS(IDR) & (1<<9))  >> 9)  \
                  | ((GPIOA->REGS(IDR) & (1<<10)) >> 9)  \
                  | ((GPIOB->REGS(IDR) & (1<<6))  >> 4)  \
                  | ((GPIOB->REGS(IDR) & (1<<7))  >> 4)  \
                  | ((GPIOA->REGS(IDR) & (1<<0))  << 4)  \
                  | ((GPIOA->REGS(IDR) & (1<<1))  << 4)  \
                  | ((GPIOA->REGS(IDR) & (1<<2))  << 4)  \
                  | ((GPIOA->REGS(IDR) & (1<<3))  << 4)))
    
//#define setWriteDir() {GP_OUT(GPIOA, CRL, 0xFFFFFFFF); }
#define setWriteDir() {GP_OUT(GPIOA, CRL, 0x0000FFFF); GP_OUT(GPIOA, CRH, 0xFF0); GP_OUT(GPIOB, CRL, 0xFF000000); }
#define setReadDir()  {GP_INP(GPIOA, CRL, 0x0000FFFF); GP_INP(GPIOA, CRH, 0xFF0); GP_INP(GPIOB, CRL, 0xFF000000); }
